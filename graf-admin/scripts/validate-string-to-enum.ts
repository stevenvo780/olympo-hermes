 ...
 *
 * Adaptado para Graf Admin (Frontend)
 */
      ...
      foundOccurrences.forEach((occ) => {
        if (valueToEnums.has(occ.stringLiteral)) {
          ...
          if (isNavigationStep) {
            if (VERBOSE) {
              console.log(
                `   ⚪ Ignorando (navigation/step): "${occ.stringLiteral}" en ${path.basename(filePath)}:${occ.line}`,
              );
            }
            return;
          }
 ...
 */
async function main() {
  console.log("🔍 Validando strings y uso de Enums en Graf Admin...\n");
