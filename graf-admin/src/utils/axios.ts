...
api.interceptors.request.use(
  async function (
    ...
    // Demo auth: when the demo api-key is present (inlined by Next.js at build
    // time), also send it so the backend accepts requests without Firebase.
  ...
  async function (error: AxiosError): Promise<unknown> {
    ...
    const isAuthError =
      ...
      (error.response.status === 401 ||
        (isProtectedRoute && error.response.status === 404) ||
        (error.response.data &&
          ...
          ((error.response.data as ApiErrorResponse).message ===
            ...
            (error.response.data as ApiErrorResponse).message ===
              "Token inválido" ||
            (error.response.data as ApiErrorResponse).message?.includes(
              "expirada",
            ) ||
            (error.response.data as ApiErrorResponse).message?.includes(
              "token",
