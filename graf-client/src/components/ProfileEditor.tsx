...
const ProfileEditor: React.FC<ProfileEditorProps> = () => {
  ...
  const [account, setAccount] = useState({ email: '', name: '', documentNumber: '', oldPassword: '' });
  ...
  useEffect(() => {
    const fetchData = async (): Promise<void> => {
      ...
      try {
        ...
        setAccount({ email: resUser.data.email || '', name: resUser.data.name || '', documentNumber: resUser.data.documentNumber || '', oldPassword: '' });
      } catch {
      }
  ...
  const submitAccount = async (e: React.FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    try {
      ...
      await api.patch('/user/me', { name: account.name, documentNumber: account.documentNumber });
      ...
      if (firebaseError.code === 'auth/requires-recent-login') {
        ...
        setErrorAccount(firebaseError.message || 'Error desconocido al actualizar la cuenta');
      }
    }
    setLoadingAccount(false);
  };
  ...
  const submitPassword = async (e: React.FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    try {
      ...
      if (firebaseError.code === 'auth/wrong-password') {
        ...
        setErrorPassword(`Error al actualizar la contraseña: ${firebaseError.message || 'desconocido'}`);
      }
    }
    setLoadingPassword(false);
  };
    ...
    <Container className="mt-5">
      ...
      <Form onSubmit={submitAccount}>
        ...
        <Form.Group controlId="formDocumentNumber" className="mb-3">
          ...
          <Form.Control type="text" name="documentNumber" value={account.documentNumber} onChange={handleAccountChange} placeholder="Ingrese su número de documento" />
        </Form.Group>
        ...
        <div className="d-flex gap-2 mb-4">
          ...
          <Button variant="secondary" onClick={() => setShowPasswordModal(true)} type="button">
            Cambiar Contraseña
          </Button>
        </div>
      </Form>
