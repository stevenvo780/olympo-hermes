 ...
 *
 * Adaptado para Graf Client (Frontend)
 */
...
// Patrones de archivos que contienen definiciones
...
// Patrones de línea que indican definición de ID
const ID_DEFINITION_PATTERNS = [
  ...
  /^\s*\*/,
  /["'][^"']*\$\{/,
  // Patrones específicos de React/Next.js
  /className:\s*["']/,
  /style:\s*["']/,
  /aria-\w+:\s*["']/,
  /data-\w+:\s*["']/,
...
// Patrones de contextos TypeScript
...
// Patrones de estados externos (payment status, etc.)
const EXTERNAL_STATUS_PATTERNS = [
  /paymentStatus\s*===\s*["'](success|pending|failed)["']/,
  /status\s*===\s*["'](success|pending|failed|declined|error)["']/,
  /case\s+["'](success|pending|failed|declined|error)["']\s*:/,
  /setPaymentStatus\(["'](success|pending|failed)["']\)/,
  /:\s*["'](success|pending|failed)["']\s*\|\s*["'](success|pending|failed)["']/,
];
...
const SYSTEM_DEFAULT_VALUES = new Set([
  ...
  "message",
  "code",
  // Valores comunes de React/Frontend
  "button",
  "div",
  "span",
  "text",
...
// Patrones para encontrar strings literales
...
/**
 * Obtiene todos los archivos TypeScript/TSX recursivamente
 */
...
/**
 * Encuentra todos los enums en el frontend
 */
function findAllEnums(frontendSrc: string): Map<string, EnumValues> {
  const enumMap = new Map<string, EnumValues>();

  const enumFiles = getAllFiles(frontendSrc);
      ...
      foundOccurrences.forEach((occ) => {
        if (valueToEnums.has(occ.stringLiteral)) {
          ...
          if (isCSSProperty) {
            if (VERBOSE) {
              console.log(
                `   ⚪ Ignorando (CSS): "${occ.stringLiteral}" en ${path.basename(filePath)}:${occ.line}`,
              );
            }
            return;
          }
          ...
          if (isConfigKey) {
            if (VERBOSE) {
              console.log(
                `   ⚪ Ignorando (config key): "${occ.stringLiteral}" en ${path.basename(filePath)}:${occ.line}`,
              );
            }
            return;
          }

          const isExternalStatus = EXTERNAL_STATUS_PATTERNS.some((pattern) =>
            pattern.test(lineContent),
          );
          if (isExternalStatus) {
            if (VERBOSE) {
              console.log(
                `   ⚪ Ignorando (external status): "${occ.stringLiteral}" en ${path.basename(filePath)}:${occ.line}`,
              );
            }
            return;
          }
 ...
 */
async function main() {
  console.log("🔍 Validando strings y uso de Enums en Graf Client...\n");

  console.log("📚 Extrayendo enums existentes...");
  const enumMap = findAllEnums(FRONTEND_SRC);
